...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-update': { kind: 'none', reason: 'Browser-side update prompt; the desktop shell owns every release action and no model surface is touched.' },
  'packages/client/ui-usage': { kind: 'none', reason: 'Browser-side dashboard over host-computed usage and balance values; registers no prompt, schema, or model request.' },
  'packages/session/usage': { kind: 'none', reason: 'Host usage service reads logged session events and the platform balance endpoint; contributes no model-facing text.' },
